...
/**
 * Explicit up-to-parent control for every Cellular Gaits page, so no page is a
 * dead end that relies on browser-back (theme C). Rendered in the shared shell
 * (layout), it derives its target from the current route:
 ...
 *   · the project frame itself → back out to the projects list.
 * The leaf label orients the visitor without needing a full breadcrumb trail.
 */
...
export function CgBreadcrumb() {
  ...
  let up: { href: string; label: string };
  if (parentTab) {
    // A sub-page → up to its hub tab.
    up = { href: parentTab.href, label: parentTab.label };
  } else if (pathname !== CG_BASE) {
    // A top-level concept tab → up to the project frame.
    up = { href: CG_BASE, label: "Cellular Gaits" };
  } else {
    // The project frame itself → out to the projects list.
    up = { href: "/#bench", label: "Projects" };
  }
