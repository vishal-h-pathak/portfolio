...
/**
 * /console/jobs — overview register.
 *
 ...
 * re-hits /api/console/dashboard/jobs?view=list&q=… (title/company ilike).
 */
...
export default function DashboardPage() {
  ...
  const fetchJobs = useCallback(async (q: string) => {
    try {
      const json = await requestJSON<{
        ...
        "GET",
        `/api/console/dashboard/jobs?view=list${q ? `&q=${encodeURIComponent(q)}` : ""}`,
      );
      setJobs(json.jobs ?? []);
  ...
  const setStatus: JobActions["setStatus"] = useCallback(
    (job, status, opts) => {
      ...
      return act.run(`status:${job.id}`, {
        ...
        perform: () =>
          requestJSON("PATCH", `/api/console/dashboard/jobs/${job.id}`, body),
    ...
    (job, reason) =>
      act.run(`status:${job.id}`, {
        ...
        perform: () =>
          requestJSON(
            "POST",
            `/api/console/dashboard/jobs/${job.id}/skip`,
            reason ? { reason } : {},
          ),
        errorLabel: "Skip",
      }),
  ...
  const tailorJob: JobActions["tailorJob"] = useCallback(
    (job) =>
      act.run(`tailor:${job.id}`, {
        perform: () =>
          requestJSON("POST", "/api/console/dashboard/runs/tailor", {
            job_id: job.id,
          }),
        errorLabel: "Tailor",
  ...
  const submitJob: JobActions["submitJob"] = useCallback(
    (job) =>
      act.run(`submit:${job.id}`, {
        ...
        perform: () =>
          requestJSON("POST", `/api/console/dashboard/jobs/${job.id}/prefill`, {}),
        errorLabel: "Submit",
  ...
  // /console/jobs/review/[job_id]. Every card opens the Match Agent panel.
