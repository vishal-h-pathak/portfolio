 ...
 *
 * The form POSTs to /api/console/dashboard/runs/tailor-manual which inserts a
 * runs row (kind='tailor_manual') and dispatches tailor-manual.yml. The
 * panel then polls /api/console/dashboard/runs?limit=50, locates its run by id,
 ...
 * /console/jobs/review/{job_id}. Low-confidence flow (Amendment 1) ends
 ...
 *
 * Mounted on /console/jobs between the header and RunsPanel so the paste-
...
export default function ManualTailorPanel() {
  ...
  const fetchRun = useCallback(async (id: string): Promise<Run | null> => {
    ...
    try {
      const res = await fetch(`/api/console/dashboard/runs?limit=50`, {
        cache: "no-store",
      });
  ...
  const dispatch = async (target: string) => {
    ...
    setSubmitError(null);
    try {
      const res = await fetch("/api/console/dashboard/runs/tailor-manual", {
        method: "POST",
...
function ResultCard({ result }: { result: ResultPayload }) {
  ...
  const linkHref =
    result.materials_url ?? result.review_url ?? `/console/jobs/review/${result.job_id}`;
