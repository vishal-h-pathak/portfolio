 ...
 *   GET  /api/console/dashboard/runs?limit=10
 *   POST /api/console/dashboard/runs/hunt | /tailor | /tailor-manual
 *
 ...
 * /console/jobs/review/[job_id] remains the only entry to the submit phase.
 *
...
const RUN_ENDPOINT: Record<RunKind, string> = {
  hunt: "/api/console/dashboard/runs/hunt",
  tailor: "/api/console/dashboard/runs/tailor",
  tailor_manual: "/api/console/dashboard/runs/tailor-manual",
};
...
export default function RunsPanel() {
  ...
  const refresh = async () => {
    ...
    try {
      const res = await fetch(`/api/console/dashboard/runs?limit=${LIST_LIMIT}`, {
        cache: "no-store",
      });
      if (!res.ok) return;
