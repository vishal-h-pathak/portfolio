...
/**
 * /console/jobs/review/[job_id] — M-6 manual-submission cockpit.
 *
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/console/dashboard/storage/sign?path=${encodeURIComponent(storagePath)}`,
        );
...
function MarkAppliedModal({
  ...
  async function submit() {
    ...
    setErr(null);
    try {
      await requestJSON("POST", `/api/console/dashboard/jobs/${jobId}/mark-applied`, {
...
export default function ReviewDetailPage() {
  ...
  async function refresh(silent: boolean) {
    ...
    try {
      const res = await fetch(`/api/console/dashboard/jobs/${jobId}`, {
        cache: "no-store",
      });
      const json = (await res.json().catch(() => ({}))) as {
        job?: Job;
  ...
  function postAction(
    ...
    void act.run(`cockpit:${path}`, {
      ...
      perform: () =>
        requestJSON("POST", `/api/console/dashboard/jobs/${job.id}/${path}`, body ?? {}),
  ...
  const resumePdfUrl = `/api/console/materials/${job.id}/resume`;
  const coverLetterPdfUrl = `/api/console/materials/${job.id}/cover_letter`;
        ...
        {showMarkApplied && (
          <MarkAppliedModal
            ...
            onApplied={() => {
              ...
              router.push("/console/jobs/review");
            }}
          />
        )}
      </main>
