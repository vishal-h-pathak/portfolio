...
/**
 * /console/jobs/review — Submit lane.
 *
 ...
 * cockpit (/console/jobs/review/[job_id]) and the resume PDF — the materials
 ...
 *
 * Reads via /api/console/dashboard/jobs?view=review-queue behind the same
 * dashboard_auth cookie that protects the rest of /console/jobs/*.
 */
...
export default function ReviewQueuePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/console/dashboard/jobs?view=review-queue", {
          cache: "no-store",
        });
  ...
  function markApplied(job: Job) {
    void act.run(`mark-applied:${job.id}`, {
      ...
      perform: () =>
        requestJSON("POST", `/api/console/dashboard/jobs/${job.id}/mark-applied`, {}),
  ...
  function skip(job: Job) {
    void act.run(`skip:${job.id}`, {
      optimistic: () => removeLocally(job.id),
      perform: () =>
        requestJSON("POST", `/api/console/dashboard/jobs/${job.id}/skip`, {
...
function Section({
  ...
  return (
    <section className="mb-8">
      <h2 className="mb-2.5 flex flex-wrap items-baseline gap-x-2 gap-y-0.5 border-b border-rule-soft pb-1.5 font-mono text-meta uppercase tracking-kicker text-ink-faint">
  ...
  return (
    <div className="space-y-1">
      {(filled !== null || skipped !== null) && (
        <div className="flex flex-wrap items-center gap-3 text-label tabular-nums">
          {filled !== null && (
      ...
      {notes && (
        <p className="line-clamp-2 text-label leading-relaxed text-ink-dim">
          {notes}
        </p>
      )}
    </div>
...
function SubmitLaneRow({
  ...
  const cockpitHref = `/console/jobs/review/${job.id}`;
  ...
  return (
    ...
    >
      <div className="min-w-0">
        ...
        <Link
          href={cockpitHref}
          className="block truncate text-note font-medium text-ink transition-colors duration-150 hover:text-amber"
        >
          {job.title}
        </Link>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-rule-soft pt-3">
        ...
        {job.resume_pdf_path && (
          <BtnLink
            href={`/api/console/materials/${job.id}/resume`}
            target="_blank"
            rel="noreferrer"
          >
            resume ↗
