...
const ITEMS: NavItem[] = [
  {
    href: "/console/jobs",
    label: "Overview",
    // Active for the bare /console/jobs route only — nested routes have
    // their own nav items.
    isActive: (p) => p === "/console/jobs",
  },
  {
    href: "/console/jobs/review",
    label: "Review",
    isActive: (p) => p.startsWith("/console/jobs/review"),
    badge: (n) => n > 0,
  },
  {
    href: "/console/jobs/stories",
    label: "Stories",
    isActive: (p) => p.startsWith("/console/jobs/stories"),
  },
  {
    href: "/console/jobs/insights",
    label: "Insights",
    isActive: (p) => p.startsWith("/console/jobs/insights"),
  },
];
...
export default function DashboardNav({
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const res = await fetch("/api/console/dashboard/jobs/action-count", {
          cache: "no-store",
        });
            ...
            return (
              <li key={item.href} className="shrink-0">
                <Link
                  ...
                  className={
                    "inline-flex items-baseline gap-1.5 border-b py-1 font-mono text-label uppercase tracking-label transition-colors duration-150 " +
                    (active
                ...
                >
                  ...
                  {showBadge && (
                    <span
                      className="border border-amber-dim px-1.5 text-meta text-amber tabular-nums"
