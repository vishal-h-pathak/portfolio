...
// ─────────────────────────────────────────────────────────────────────────────
// MERIDIAN — ARCHIVED EXHIBIT
//
// The retired MERIDIAN telemetry console, resurrected from git history
// (app/console/meridian/page.tsx, deleted in the P6 console consolidation
// at 1491e5d) and frozen as a public post-mortem artifact. Changes from
// the original, all in service of "archived, not live":
//   - the live-data hook (/api/console/meridian + Supabase merge) is
//     removed — that plumbing is dead; nothing here pretends otherwise;
//   - the synthetic demo feed the console always carried is now seeded
//     (mulberry32) so the exhibit renders identically on every load;
//   - LIVE badges and pulse animations are gone, the colophon is frozen,
//     and an archive banner states MERIDIAN's actual final record.
// The numbers below are the console's demo telemetry, not MERIDIAN's real
// record — the banner carries the real record (1 trade in 2 months).
// ─────────────────────────────────────────────────────────────────────────────
import { useState, useEffect, useRef, useMemo, useId } from "react";

// Deterministic PRNG so the frozen exhibit is byte-stable across loads
// (and across server render vs hydration).
// Fixed-locale, fixed-zone timestamp label — toLocaleString would render
// differently on the server and in each visitor's timezone, which a frozen
// exhibit must not do.
function fmtTsUTC(d: Date): string {
...
// ─────────────────────────────────────────────────────────────────────────────
// 5. DATA GENERATION — the console's synthetic demo feed, now seeded/frozen
// ─────────────────────────────────────────────────────────────────────────────
function generateMeridianData() {
  const rng = mulberry32(0x4d455249); // "MERI" — fixed forever
  ...
  const decisions: any[] = [];
  const startTs = new Date('2026-04-05T09:30:00Z');
  let portfolio = 100000;
  const portfolioSeries: { t: number; v: number }[] = [{ t: startTs.getTime(), v: portfolio }];

  for (let i = 0; i < 48; i++) {
    const t = new Date(startTs.getTime() + i * 3600_000 * 6 + rng() * 3600_000 * 2);
    ...
    const price = basePrices[sym] * (1 + (rng() - 0.5) * 0.08);
    const ret = action === 'HOLD' ? 0 : (action === 'BUY' ? 1 : -1) * (rng() * 0.06 - 0.015) * (confidence === 'HIGH' ? 1.4 : confidence === 'LOW' ? 0.5 : 1);
    ...
    decisions.push({
      ...
      ts_label: fmtTsUTC(t),
      action, symbol: sym,
      instrument: rng() > 0.7 ? 'option' : 'equity',
      qty: Math.round(50 + rng() * 400),
      price: +price.toFixed(2),
      confidence, theme: themes[Math.floor(rng() * themes.length)],
      cycle: rng() > 0.8 ? 'FAST' : 'STANDARD',
      reasoning: rowReasonings[Math.floor(rng() * rowReasonings.length)],
      quant_score: +qs.toFixed(2), quant_grade: grade,
      component_scores: {
        rsi: +(rng() * 0.6 + 0.3).toFixed(2),
        volume: +(rng() * 0.6 + 0.3).toFixed(2),
        ma_cross: +(rng() * 0.6 + 0.3).toFixed(2),
        iv_rank: +(rng() * 0.6 + 0.3).toFixed(2),
        momentum: +(rng() * 0.6 + 0.3).toFixed(2),
        options_structure: +(rng() * 0.6 + 0.3).toFixed(2),
        price_action: +(rng() * 0.6 + 0.3).toFixed(2),
        fundamentals: +(rng() * 0.6 + 0.3).toFixed(2),
      },
      brain_agreement: ['both_agree', 'quant_override', 'narrative_override', 'conflict'][Math.floor(rng() * 4)],
      executed: rng() > 0.15,
      mode: 'paper', portfolio_value: +portfolio.toFixed(2),
      geo_focus: rng() > 0.5 ? 'semiconductor supply chain' : 'iran strait tension',
      sentiment: ['cautiously bullish', 'bearish', 'neutral', 'bullish', 'cautiously bearish'][Math.floor(rng() * 5)],
  ...
  for (let d = 0; d < 30; d++) {
    ...
    ['NVDA', 'INTC', 'MU', 'AMD'].forEach((s, i) => {
      row[s] = 50 + Math.sin(d * 0.3 + i) * 18 + rng() * 8 - 4;
    });
  ...
  for (let d = 0; d < 30; d++) {
    geoSeries.push({
      day: d,
      semi: Math.min(100, 30 + d * 1.4 + Math.sin(d * 0.4) * 8 + rng() * 6),
      iran: Math.min(100, 45 + Math.cos(d * 0.25) * 15 + d * 0.3 + rng() * 5),
    });
  }
  ...
  for (let d = 0; d < 24; d++) {
    ragSeries.push({
      hour: d,
      news: Math.round(12 + rng() * 30),
      filings: Math.round(2 + rng() * 8),
      social: Math.round(20 + rng() * 60),
    });
  }
  ...
  const heatmap = tickers.map((tk: string) => ({
    ...
    days: Array.from({ length: 14 }, (_, i) => {
      ...
      return { day: i, action, conf: action ? 0.3 + rng() * 0.65 : 0 };
    }),
  }));
...
// ─────────────────────────────────────────────────────────────────────────────
// 6. LIVE DATA HOOK — removed. The console used to merge real decisions
// from /api/console/meridian (Supabase) over the demo feed; that route and
// the engine behind it were retired with MERIDIAN. An archive does not
// fetch.
// ─────────────────────────────────────────────────────────────────────────────
...
// Archived: the pulse ring is gone — a static lamp implies no liveness.
function StatusLamp({ color = C.text3, size = 7 }: any) {
  return (
...
function AnalystWorkspace({ D }: any) {
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24, paddingBottom: 24, borderBottom: `1px solid ${C.border}`, flexWrap: 'wrap' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              <Wordmark />
              <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center', padding: '3px 8px', border: `1px solid ${C.amber}44`, background: 'rgba(245,158,11,0.12)', borderRadius: 2, fontFamily: 'var(--mono)', fontSize: 9.5, color: C.amber, letterSpacing: '0.2em' }}>
                <StatusLamp size={5} color={C.amber} /> ARCHIVED
              </span>
            </div>
...
function NarrativeScroll({ D }: any) {
  const [selected, setSelected] = useState(D.decisions[0]);
  const pnl = D.stats.portfolio - D.stats.startPortfolio;
  const pnlPct = pnl / D.stats.startPortfolio * 100;
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 1200, margin: '0 auto', padding: '40px 32px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 20, borderBottom: `2px solid ${C.text1}` }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ display: 'inline-flex', gap: 6, alignItems: 'center', fontFamily: 'var(--mono)', fontSize: 10.5, color: C.text3, letterSpacing: '0.2em' }}>
              <StatusLamp size={6} /> ARCHIVED · SYNTHETIC · PAPER
            </div>
        ...
        <div style={{ paddingTop: 40, marginTop: 40, borderTop: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--mono)', fontSize: 10, color: C.text4, letterSpacing: '0.16em' }}>
          ...
          <span>FROZEN JUL 2026 · ARCHIVE</span>
        </div>
      </div>
    </div>
  );
...
// ─────────────────────────────────────────────────────────────────────────────
// 12. MAIN PAGE — archive banner + view switcher
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 32px' }}>
        ...
        <p style={{ fontSize: 13.5, lineHeight: 1.65, color: C.text2, margin: '14px 0 0', maxWidth: 860 }}>
          ...
          system that ran on paper from April to July 2026, deliberated
          beautifully, and produced <span style={{ color: C.text1 }}>exactly one trade in
          two months</span>. Most of its HOLDs turned out to be cascading pipeline
          failures wearing a poker face. The honest review — sophisticated
          reasoning, no pre-registered edge — is the lesson SOLITON was
          rebuilt on.
        </p>
        <p style={{ fontSize: 13.5, lineHeight: 1.65, color: C.text3, margin: '10px 0 0', maxWidth: 860 }}>
          The console is preserved as it shipped. Its live plumbing is dead;
          what renders below is the seeded <em>synthetic demo feed</em>{' '}
